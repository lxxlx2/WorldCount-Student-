...
-- DML：修改一门课程的学分，再删除专门用于演示的选课记录。
UPDATE tblCourse SET credits = 4 WHERE course_id = 'CS0005';
...
-- 查询 1：所有学生，按学号排序。
SELECT * FROM tblStudent ORDER BY student_id;

-- 查询 2：选修“数据库原理”的学生姓名和成绩（多表连接）。
SELECT s.student_name AS 姓名, sc.usual_score AS 平时成绩,
...
FROM tblStudent s
LEFT JOIN tblScore sc ON sc.student_id = s.student_id
GROUP BY s.student_id, s.student_name
ORDER BY s.student_id;

-- 查询 4：总评成绩达到 90 分的学生及课程。
SELECT s.student_id AS 学号, s.student_name AS 姓名,
       c.course_name AS 课程名, sc.total_score AS 总评成绩
FROM tblScore sc
JOIN tblStudent s ON s.student_id = sc.student_id
JOIN tblCourse c ON c.course_id = sc.course_id
...
-- 查询 5：按院系统计有选课记录的学生人数。
SELECT s.department AS 院系,
       COUNT(DISTINCT sc.student_id) AS 选课人数
FROM tblStudent s
JOIN tblScore sc ON sc.student_id = s.student_id
