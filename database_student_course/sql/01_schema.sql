...
CREATE TABLE tblStudent (
  ...
  department VARCHAR(50) COMMENT '所在院系',
...
CREATE TABLE tblCourse (
  ...
  course_type ENUM('必修', '选修', '公选') COMMENT '课程类别',
  offering_department VARCHAR(50) COMMENT '开课院系',
  CONSTRAINT chk_course_credits CHECK (credits BETWEEN 1 AND 6)
) ENGINE=InnoDB COMMENT='课程表';

CREATE TABLE tblScore (
  enrollment_id INT AUTO_INCREMENT PRIMARY KEY COMMENT '选课编号',
  student_id CHAR(10) NOT NULL COMMENT '学号',
  course_id CHAR(6) NOT NULL COMMENT '课程号',
  usual_score INT COMMENT '平时成绩',
  final_score INT COMMENT '期末成绩',
  total_score INT
    GENERATED ALWAYS AS (ROUND(usual_score * 0.30 + final_score * 0.70, 0)) STORED
    COMMENT '总评成绩：平时成绩30%+期末成绩70%',
